...
# make table with only 4 columns

bak_mtOI <- mtX_OI
mtX_OI <- bak_mtOI |> select(category, strand, motif, start) |> distinct()

table(mtX_OI$category, mtX_OI$strand)

library(ggseqlogo)

# create sequence logos for each strand and each category and overall
# citation: "Wagih, Omar. ggseqlogo: a versatile R package for drawing sequence logos. Bioinformatics (2017). https://doi.org/10.1093/bioinformatics/btx469 "

ggseqlogo(mtX_OI$motif, method="bits")+
  ggtitle("Overall, both strands, 1729 obeservations")

# split into categories, but still for both strands together

# filter for category "intergenic"
intergenic <- filter(mtX_OI, category == "intergenic")
ggseqlogo(intergenic$motif, method="bits") +
  ggtitle("Intergenic, both strands, 785 observations")

# in the gene
gene <- filter(mtX_OI, category == "gene")
ggseqlogo(gene$motif, method="bits") +
  ggtitle("In gene, both strands, 677 observations")

# downstream
downstream <- filter(mtX_OI, category == "downstream")
ggseqlogo(downstream$motif, method="bits") +
  ggtitle("Downstream, both strands, 138 observations")

# upstream
upstream <- filter(mtX_OI, category == "upstream")
ggseqlogo(upstream$motif, method="bits") +
  ggtitle("Upstream, both strands, 129 observations")


# ------

# filter the data by strand
mtx_positive <- filter(mtX_OI, strand == "+")
mtx_negative <- filter(mtX_OI, strand == "-")


# overall
ggseqlogo(mtx_positive$motif, method= "bits") +
  ggtitle("Positive Strand, Overall, 863 observations")
ggseqlogo(mtx_negative$motif, method= "bits") +
  ggtitle("Negative Strand, Overall, 866 observations")
...
# filter for category "intergenic"
intergenic_positive <- filter(mtx_positive, category == "intergenic")
ggseqlogo(intergenic_positive$motif, method="bits") +
  ggtitle("Intergenic, positive strand, 386 observations")

# in the gene
gene_positive <- filter(mtx_positive, category == "gene")
ggseqlogo(gene_positive$motif, method="bits")+
  ggtitle("In gene, positive strand, 347 observations")

# downstream
downstream_positive <- filter(mtx_positive, category == "downstream")
ggseqlogo(downstream_positive$motif, method="bits") +
  ggtitle("Downstream, positive strand, 71 observations")

# upstream
upstream_positive <- filter(mtx_positive, category == "upstream")
ggseqlogo(upstream_positive$motif, method="bits") +
  ggtitle("Upstream, positive strand, 59 observations")
...
# filter for category "intergenic"
intergenic_negative <- filter(mtx_negative, category == "intergenic")
ggseqlogo(intergenic_negative$motif, method="bits") +
  ggtitle("Intergenic, negative strand, 399 observations")
#ggsave("MyPlotSavedFromRdirectly.png")

# in the gene
gene_negative <- filter(mtx_negative, category == "gene")
ggseqlogo(gene_negative$motif, method="bits") +
  ggtitle("In gene, negative strand, 330 observations")

# downstream
downstream_negative <- filter(mtx_negative, category == "downstream")
ggseqlogo(downstream_negative$motif, method="bits") +
  ggtitle("Downstream, negative strand, 67 observations")

# upstream
upstream_negative <- filter(mtx_negative, category == "upstream")
ggseqlogo(upstream_negative$motif, method="bits") +
  ggtitle("Upstream, negative strand, 70 observations")

#######
# how many times position 29 does not have a T

# where is not a "T" on position 29
keinT <- mtx_positive %>% filter(substr(motif, 29, 29) != "T")
keinT

keinTnegativ <- mtx_negative %>% filter(substr(motif, 29, 29) != "T")


noT <-bak_mtOI%>% filter(substr(motif, 29, 29) != "T")
