#### Omics BIO253
####HEATMAP + Dendrogramm --> sortes clones vs. sorted conditions

###%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
####Spalten orden so das alle PASN zusammen und dann alle TSB zusammen!
#####Spalten nach Klone ordnen
##darstellen als heatmap und Dendogram uf de rows????
##STX --> den log 2 nehmen
##oder STX nicht im log2
####delta vom delta nehmen comparing within clones!
###!!!BUT WE CAN NOT COMPARE JE2 and 6850 directly only with respective change towards their Ancestor==REference
###%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

################################################################################
### HEATMAP ohne Dendrogram --> sortiert nach Clone + missing values=grey box###
################################################################################
library(tidyverse)
library(pheatmap)
...
##################################################################
###Heatmap ohne STX, ohne Dendogram --> SORTIERT NACH TSB/PASN####
##################################################################
library(tidyverse)
library(pheatmap)
...
########################################################################
### Heatmap + Dendrogram  STRAINS SEPARATED --> 2 individual Heatmaps###
########################################################################
library(tidyverse)
library(pheatmap)
